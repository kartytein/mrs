-- ===== ПОЛНЫЙ СКРИПТ С ПОДДЕРЖКОЙ ОСТРОВА PREHISTORICISLAND (ВЫХОД ИЗ ЛОДКИ НА 10 МИНУТ) =====
...
local islandMode = false  -- активен ли режим острова
...
local function forceSit()
    if islandMode then
        print("[ISLAND] Режим острова активен, посадка отключена")
        return
    end
...
-- ========== 8. МОНИТОР ОСТРОВА PREHISTORICISLAND ==========
...
-- Мониторинг появления/исчезновения острова (отдельный поток)
task.spawn(function()
...
-- ========== 10. ГЛАВНЫЙ ЦИКЛ (ПЕРВИЧНЫЙ ЗАПУСК) ==========
task.spawn(function()
    selectMarines()
    task.wait(2)
...
print("Скрипт запущен. При появлении Prehistoricisland персонаж выйдет из лодки и не будет возвращаться 10 минут (либо до исчезновения острова).")
