-- ============================================================
-- ПОЛНЫЙ СКРИПТ
-- 1) Проверка на конфликтующие ники (свой игнорируется) → перезаход
-- 2) Команда -> инвентарь -> сервер
-- 3) Телепорт по JobId (если нужно)
-- 4) Ресет фруктов
-- 5) Движение к waypoint -> к сиденью TradeTable (BodyVelocity)
-- 6) Авто-трейд
-- Отключение коллизий работает ТОЛЬКО во время трейда
-- ============================================================
...
-- ====================== СПИСОК НИКОВ ДЛЯ ИГНОРА ======================
local IGNORE_NICKNAMES = {
    "PhilipHenry11750",
    "EdwardThornton360",
}
local SERVER_CHECK_INTERVAL = 10

local function findConflictingPlayer()
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= player then
            for _, banned in ipairs(IGNORE_NICKNAMES) do
                if plr.Name == banned then return plr.Name end
            end
        end
    end
    return nil
...
task.spawn(function()
    while true do
        local conflict = findConflictingPlayer()
        if conflict then
            warn("[Ignore] Найден '" .. conflict .. "', перезаход...")
            pcall(function()
...
-- ====================== ОТКЛЮЧЕНИЕ КОЛЛИЗИЙ (только во время трейда) ======================
task.spawn(function()
    while true do
...
-- 4. ВКЛЮЧАЕМ ОТКЛЮЧЕНИЕ КОЛЛИЗИЙ
collisionsDisabled = true
print("[Collisions] Отключение коллизий включено.")
            ...
            if not allItemsAdded then
                ...
                if not isSeated(mySeat) then
                    ...
                    if done then
                        ...
                        print("[Collisions] Отключение коллизий выключено.")
                        tradeCompleted = true
                        break
                    else
                        if not resetSeatAndWait(mySeat, tablePos) then break end
