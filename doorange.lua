...
-- ============================================================
local BLACKLIST = {
    "ТвойНикЗдесь",        -- собственный ник (игнорируется)
    "НикВрага1",
    "НикВрага2",
    -- добавляй сюда
}
...
local function doBeltScan()
    -- Открытие меню / инвентаря (если не открыт)
...
-- ============================================================
task.spawn(function()
    task.wait(8)  -- даём хабу время загрузиться, а ботам — не мешать старту
...
-- ============================================================
-- РЕЖИМ "NO BELT": dragon talon -> island -> 2,4 -> mastery>500 -> 6,1
-- ============================================================
...
-- ============================================================
-- РЕЖИМ "YELLOW/WHITE": держим 6,1 включённой, ждём пояс
-- ============================================================
local function runYellowWhiteMode()
    ...
    local lastSeen = State.beltChangedAt

    while State.running do
        -- если пояс сменился — выходим, диспетчер перекинет
        if State.currentBelt ~= modeAtStart then
            print("[Mode:YellowWhite] пояс изменился на " .. State.currentBelt .. " — переключение")
            return
        end

        -- если давно не было обновлений — хопаем
        ...
        -- подстраховка: раз в минуту держим 6,1 включённой
        pcall(function() setOption(TAB_MAIN, OPT_MAIN, true) end)
        task.wait(60)
    end
end
...
-- ============================================================
-- РЕЖИМ "ORANGE": трейд (полный пользовательский скрипт)
-- ============================================================
local function runOrangeMode()
    print("[Mode:Orange] трейд-режим запущен")

    -- На время трейда сканер пояса пауза (иначе будет открывать инвентарь)
    State.beltScanPaused = true

    -- ===== ЛОКАЛЬНЫЕ НАСТРОЙКИ ТРЕЙДА =====
    ...
    -- После трейда: возобновляем скан пояса и просим диспетчер пере-оценить режим
    State.beltScanPaused = false
    State.currentBelt = "Unknown"  -- форсируем пересбор режима в главном цикле
    task.wait(1)
...
-- ГЛАВНЫЙ ДИСПЕТЧЕР
-- ============================================================
-- Стартовая задержка, чтобы хаб и игра прогрузились
task.wait(5)
...
while State.running do
    ...
    elseif belt == "None" then
        ...
        -- Прочие пояса (Green/Blue/...) — пока не обрабатываем, просто ждём
