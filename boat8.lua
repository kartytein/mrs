-- ===== ФИНАЛЬНЫЙ СКРИПТ С ПРАВИЛЬНЫМИ ПАРАМЕТРАМИ (ПО ЛОГАМ ЭТАЛОННОГО СКРИПТА) =====
...
-- ========== МАГНИТ (ПРАВИЛЬНЫЙ, КАК В ЭТАЛОНЕ) ==========
local function magnet()
    ...
    if dist > 0.3 then
        ...
        -- достаточно близко – фиксируем позицию, игра сама посадит
...
task.spawn(function()
    while true do
        task.wait(0.05)   -- частота как в эталоне
        if islandMode then
            -- во время острова ничего не делаем
            continue
        end
...
-- Первичная посадка (если лодка есть и персонаж ещё не сидит)
task.spawn(function()
    while true do
        task.wait(0.5)
...
print("Финальный скрипт запущен. Магнит работает с частотой 0.05 сек, скорость 150 студий/сек, как в эталоне.")
