...
-- + Дебагер: каждые 60 секунд перезапускает кнопку
-- ============================================================
...
local RESTART_INTERVAL = 60 -- интервал перезапуска в секундах
...
-- Перезапустить опцию (выключить, если включена, затем включить)
local function restartOption()
    local state = getOptionState(TAB, OPT)
    if state == "on" then
        -- выключаем
        local root = getRoot()
        ...
        if not optionBtn then return false end
        fireSequence(optionBtn) -- клик для выключения
        task.wait(0.2)
        fireSequence(optionBtn) -- клик для включения
        return true
...
-- Основной цикл: держать кнопку включённой + перезапуск каждые 60 сек
log("Скрипт запущен, следим за кнопкой 6,1, перезапуск каждые " .. RESTART_INTERVAL .. " сек")
local lastRestartTime = tick()
while true do
    local state = getOptionState(TAB, OPT)
    if state == "off" then
    ...
    -- Дебагер: перезапуск по таймеру
    if tick() - lastRestartTime >= RESTART_INTERVAL then
        log("Перезапуск кнопки по таймеру...")
        local ok = restartOption()
        if ok then
            log("Кнопка перезапущена")
        else
            log("Не удалось перезапустить кнопку")
        end
