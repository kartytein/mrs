-- ===== ФИНАЛЬНЫЙ СКРИПТ С ПАРАМЕТРАМИ ИЗ ЛОГОВ (БЫСТРЫЙ МАГНИТ) =====
...
local function moveStep(targetPos, speed, keepY)
    ...
    local step = 0.02                     -- интервал как в логах
...
local function sitOnSeat(seat, hrp, hum)
    ...
    moveStep(target, 300, true)   -- скорость 300 как в логах
    hum.Sit = true
    task.wait(0.3)
end
...
-- ========== 5. БЫСТРЫЙ МАГНИТ (ПО ЛОГАМ: скорость 300, интервал 0.02 сек) ==========
...
print("Финальный скрипт запущен. Магнит работает с параметрами из логов (скорость 300, интервал 0.02 сек).")
