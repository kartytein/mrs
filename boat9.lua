-- ===== ФИНАЛЬНЫЙ СКРИПТ С ПОДДЕРЖКОЙ ОСТРОВА И DRAGON EGG =====
...
local function updateBoatMovement()
    ...
    if not isSitting or islandMode then return end
    ...
    if duration > 0 then
        ...
        currentTween.Completed:Connect(function()
            currentTween = nil
            if not islandMode then updateBoatMovement() end
        end)
    end
end
...
-- ===== ОЖИДАНИЕ ПОЯВЛЕНИЯ И ИСЧЕЗНОВЕНИЯ DRAGON EGG =====
local function waitForDragonEgg()
    local egg = nil
    -- Ждём появления яйца (до 60 секунд)
    for _ = 1, 60 do
        egg = workspace:FindFirstChild("Map.Prehistoricisland.Core.SpawnedDragonEggs.DragonEgg")
        if egg then break end
        task.wait(1)
    end
    if not egg then
        print("[ISLAND] Dragon Egg не появился, возвращаемся")
        return
    end
    print("[ISLAND] Dragon Egg появился, ожидаем исчезновения...")
    while egg and egg.Parent do
        task.wait(0.5)
    end
    print("[ISLAND] Dragon Egg исчез")
end
...
task.spawn(function()
    while not stopScript do
        ...
        if island and not islandMode then
            print("[ISLAND] Prehistoricisland обнаружен, переключение в режим острова")
            islandMode = true

            -- Останавливаем лодку и выходим
            stopBoat()
            exitBoat()
            task.wait(1)

            -- Перемещаемся к острову
            ...
            islandMode = false
            needToSit = true
            print("[ISLAND] Режим острова завершён")
        end
        task.wait(1)
    end
end)
...
print("Скрипт запущен. Режим острова активен.")
