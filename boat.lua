-- ============================================================
-- ИСПРАВЛЕНИЕ: обернули загрузку хаба в pcall с логированием
-- Запускаем наш скрипт немедленно, хаб — с задержкой и защитой
-- ============================================================

-- Сначала наш основной скрипт
local success, err = pcall(function()
    ...
    local function setOptionState(tabIndex, optIndex, desiredState, conflictTab, conflictOpt)
        ...
        -- сначала выключаем конфликтующую опцию, если требуется
...
if not success then
    warn("[AutoFarm] Ошибка в главном скрипте:", err)
end

-- Загружаем хаб с защитой от ошибок
task.delay(5, function()
    local ok, result = pcall(function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/Omgshit/Scripts/main/MainLoader.lua"))()
    end)
    if not ok then
        warn("[AutoFarm] Ошибка загрузки хаба:", result)
    else
