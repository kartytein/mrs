-- ============================================================
-- AutoFarm Prehistoric Island + Boat Movement (FIX task.spawn)
-- Заменён task.spawn на spawn для совместимости с экзекуторами,
-- где task библиотека недоступна. Коллизии отключаются постоянно.
-- ============================================================
...
repeat wait(0.5) until waitForInterface()
...
local function getOptionState(tabIndex, optIndex)
    ...
    local function findTab(p)
        ...
        for _, c in ipairs(p:GetChildren()) do
            if c:IsA("TextButton") or c:IsA("ImageButton") then
                tabCount = tabCount + 1; if tabCount == tabIndex then tabButton = c return end
            end
            findTab(c)
        end
    end
    findTab(tabsScroll) if not tabButton then return nil end
    fireSequence(tabButton) wait(0.3)
    ...
    for _, c in ipairs(container:GetChildren()) do
        if c.Name == "Option" and c.Visible and (c:IsA("TextButton") or c:IsA("ImageButton")) then
            optCount = optCount + 1; if optCount == optIndex then optionBtn = c break end
        end
    end
...
local function setOptionState(tabIndex, optIndex, desiredState, conflictTab, conflictOpt)
            ...
            if cfRoot then
                ...
                if cfTabsScroll then
                    ...
                    local function findCfTab(p)
                        ...
                        for _, c in ipairs(p:GetChildren()) do
                            if c:IsA("TextButton") or c:IsA("ImageButton") then
                                cfTabCount = cfTabCount + 1; if cfTabCount == conflictTab then cfTabBtn = c return end
                            end
                            findCfTab(c)
                        end
                    end
                    findCfTab(cfTabsScroll) if cfTabBtn then fireSequence(cfTabBtn) wait(0.3) end
                end
    ...
    fireSequence(optionBtn) wait(0.1)
    return true
end
...
local function startBoatCollisionLoop()
    ...
    collisionThread = spawn(function()  -- spawn вместо task.spawn
        while boat do
            pcall(function()
...
local function stopBoatCollisionLoop()
    if collisionThread then
        -- spawn возвращает thread? Нет, spawn ничего не возвращает, нужно сохранять.
        -- Но мы уже не можем cancel поток, созданный spawn. Чтобы избежать утечек,
        -- просто обнулим ссылку, а поток сам завершится, когда boat станет nil.
        collisionThread = nil
    end
end
...
local function stopMove()
    moving = false
    if moveThread then
        -- отмена moveThread невозможна, если он создан через spawn, но он сам проверяет moving
        moveThread = nil
    end
...
local function isAliveAndSeated()
    local char = player.Character if not char then return false end
    local hum = char:FindFirstChild("Humanoid") return hum and hum.Health > 0 and hum.Sit and hum.SeatPart == seat
end

local function startMove()
    if moving or not rootPart then return end
    moving = true
    startBoatCollisionLoop()
    moveThread = spawn(function()  -- spawn вместо task.spawn
        -- выравнивание высоты
        if rootPart and rootPart.Parent then
            pcall(function()
                local p = rootPart.Position
                if math.abs(p.Y - TARGET_Y) > 0.5 then rootPart.CFrame = CFrame.new(p.X, TARGET_Y, p.Z) end
            end)
        end
        if not isAliveAndSeated() then stopMove(); return end
        if not ensureBV() then stopMove(); return end
        while moving do
            if not isAliveAndSeated() then stopMove(); break end
            if rootPart then
                local p
                pcall(function() p = rootPart.Position end)
                if p then
                    if math.abs(p.Y - TARGET_Y) > 0.5 then
                        pcall(function() rootPart.CFrame = CFrame.new(p.X, TARGET_Y, p.Z) end)
                    end
                    if p.X <= X_MIN and dir == -1 then
                        dir = 1; if not ensureBV() then stopMove(); break end
            ...
            if bv and bv.Parent then
                pcall(function() local v = bv.Velocity; bv.Velocity = Vector3.new(v.X, v.Y - 0.0001, v.Z - 0.0001) end)
            end
            wait(0.05)
        end
    end)
...
local function isInBoat()
    local char = player.Character if not char then return false end
    local hum = char:FindFirstChild("Humanoid") if not hum or not hum.Sit or not hum.SeatPart then return false end
...
while true do
    ...
    if state == "INIT" then
        if not waitForInterface() then wait(0.5) continue end
    ...
    elseif state == "WAITING_FOR_BOAT_OPTION" then
        ...
        elseif cur == nil then wait(1) continue end
        state = "WAITING_FOR_BOARD_BOAT" wait(0.2)
    ...
    elseif state == "GOING_TO_ISLAND" then
        ...
        if hrp and (hrp.Position - islandPos).Magnitude <= 100 then state = "WAITING_ALL_NEAR" else wait(0.3) end
    elseif state == "WAITING_ALL_NEAR" then
        ...
        if allPlayersNearIsland(islandPos) then state = "RETURN_TO_BOAT" else wait(1) end
