-- ============================================================
-- AutoFarm Prehistoric Island + Boat Movement (FIX SPAWN)
-- Заменён spawn на coroutine.resume(coroutine.create(...)) для 
-- обхода ошибки "attempt to call a nil value" (spawn отсутствует).
-- Коллизии отключаются постоянно.
-- ============================================================

-- Безопасная обёртка для запуска функции в отдельном потоке
local function startThread(f)
...
local collisionActive = false -- флаг, чтобы не запускать несколько циклов коллизий
...
local function startBoatCollisionLoop()
    ...
    startThread(function()
        while collisionActive and boat do
            pcall(function()
...
local function startMove()
    ...
    moveThread = true  -- флаг, что поток запущен; сам поток ниже
    startThread(function()
        -- выравнивание высоты
